...
// // Helper function to extract table name from a condition
...
//         *column = strdup(dot + 1);
//     } else {
//         // Just column name, default table to NULL
//         *table = NULL;
//         *column = strdup(expr);
//     }
// }

// // Helper function to check if a condition can be pushed down
// int can_push_to_table(const char *condition, const char *table_name) {
//     // Parse the condition to see if it references only this table
...
//     extract_table_column(left_side, &table, &column);
    
//     int result = 0;
//     if (table && strcmp(table, table_name) == 0) {
//         result = 1;
//     }
...
//     // If this is a selection node (σ)
//     if (node->operation && strcmp(node->operation, "σ") == 0) {
//         // If child is a join node
//         if(node->child)printf("Pushing down selection '%s'\n", node->child->operation);
//         if (node->child && node->child->operation && strcmp(node->child->operation, "table") == 0 && node->child->next && strcmp(node->child->next->operation, "⨝") == 0) {
//             // Try to push the condition to one of the tables in the join
//             // This is a simplified approach
//             Node *join_node = node->child;
//             Node *table_node = join_node->child; // Right table in join
            
//             if (can_push_to_table(node->arg1, table_node->arg1)) {
//                 printf("Pushing condition '%s' down to table '%s'\n", node->arg1, table_node->arg1);
                
//                 // Create a new selection node for the pushed condition
//                 Node *new_selection = new_node("σ", node->arg1, NULL);
//                 new_selection->child = table_node;
                
//                 // Update join to use the new selection
//                 join_node->child = new_selection;
                
//                 // Remove the original selection node
//                 Node *result = join_node;
//                 free(node);
//                 return result;
//             }
//         }
...
//     if (node->child) {
//         node->child = push_down_selections(node->child);
//     }
//     if (node->next) {
//         node->next = push_down_selections(node->next);
//     }
...
//     // Simplified parsing, assumes format "table1.col1 = table2.col2"
//     char condition_copy[100];
//     strcpy(condition_copy, condition);
...
//     if (!left_stats || !right_stats) return 1000000; // Very high cost if stats not available
...
//     // to find the optimal join order. This is a placeholder.
...
//         // Simplified implementation - in a real system, we'd use dynamic programming
//         // to find the optimal join order
//         printf("Join reordering would be applied here\n");
        
//         // Process children recursively
//         if (node->child) {
//             node->child = reorder_joins(node->child);
//         }
//         if (node->next) {
//             node->next = reorder_joins(node->next);
//         }
//     } else {
//         // Process children recursively
//         if (node->child) {
...
//         // Find the two tables being joined
...
//         // Selectivity would ideally be calculated from the join condition
//         double selectivity = 0.1; // Default assumption
        
//         return left_cost + right_cost + 
//                (left_stats->row_count * right_stats->row_count * selectivity);
//     }
    
//     // This is a simplified cost model. In reality, you'd consider:
//     // 1. I/O cost
//     // 2. CPU cost
//     // 3. Memory usage
//     // 4. Network transfer (for distributed systems)
...
//         // Selection cost is cost of child + small CPU cost
//         double child_cost = estimate_cost(node->child);
//         return child_cost + 10;  // 10 is arbitrary CPU cost
//     }
//     else if (strcmp(node->operation, "⨝") == 0) {
//         // Join cost depends on algorithm and table sizes
//         // Here we assume nested loop join for simplicity
//         double left_cost = estimate_cost(node->child);
//         double right_cost = estimate_cost(node->next);
//         return left_cost * right_cost + left_cost + right_cost;
//     }
//     else if (strcmp(node->operation, "π") == 0) {
//         // Projection cost is cost of child + small CPU cost
//         double child_cost = estimate_cost(node->child);
//         return child_cost + 5;  // 5 is arbitrary CPU cost
//     }
...
// // Helper function to check if a condition can be pushed down to a table
