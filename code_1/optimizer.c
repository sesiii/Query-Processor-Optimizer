...
//             metrics.io_cost = stats->row_count * 1.0; // 1 unit per row scanned
//             metrics.cpu_cost = 0.0; // No processing for base table
//         } else {
...
//         metrics.result_size = child_metrics.result_size * selectivity;
//         metrics.io_cost = child_metrics.io_cost; // Same data scanned
//         metrics.cpu_cost = child_metrics.cpu_cost + child_metrics.result_size * 0.01; // Comparison cost
...
//     if (strcmp(node->operation, "⨝") == 0) {
//         Node *left_table = node->child;
//         Node *right_table = left_table ? left_table->next : NULL;
        
//         if (!left_table || !right_table) return metrics;
...
//         double selectivity = 0.1;
//         if (node->arg1 && strcmp(left_name, "unknown") != 0 && strcmp(right_name, "unknown") != 0) {
//             char *left_table_name, *left_col, *right_table_name, *right_col;
...
//         metrics.result_size = left_metrics.result_size * right_metrics.result_size * selectivity;
//         metrics.io_cost = left_metrics.io_cost + right_metrics.io_cost;
//         metrics.cpu_cost = left_metrics.cpu_cost + right_metrics.cpu_cost + 
//                           (left_metrics.result_size * right_metrics.result_size * 0.01); // Join comparison cost
//         return metrics;
//     }
    
//     if (strcmp(node->operation, "π") == 0) {
//         CostMetrics child_metrics = estimate_cost(node->child);
//         metrics.result_size = child_metrics.result_size; // Projection doesn't change row count
//         metrics.io_cost = child_metrics.io_cost;
//         metrics.cpu_cost = child_metrics.cpu_cost + child_metrics.result_size * 0.005; // Projection overhead
//         return metrics;
//     }
...
//     if (strcmp(node->operation, "table") == 0) {
//         if (node->arg2)
//             printf("table(%s AS %s) [size=%d, io=%.1f, cpu=%.1f]\n", 
//                    node->arg1, node->arg2, metrics.result_size, metrics.io_cost, metrics.cpu_cost);
//         else
...
//         printf("AST after selection push-down:\n");
//         print_ast(optimized_root, 0);
//     }

//     if (enable_projection_pushdown) {
//         printf("Applying projection push-down...\n");
//         optimized_root = push_down_projections(optimized_root);
//         printf("AST after projection push-down:\n");
//         print_ast(optimized_root, 0);
//     }
...
//             if (left_table->next && left_table->next->operation && 
//                 strcmp(left_table->next->operation, "table") == 0) {
...
//             metrics.cpu_cost = 0.0;
//         }
//         if (debugkaru) printf("Table %s: size=%d, io=%.1f, cpu=%.1f\n",
//                               node->arg1, metrics.result_size, metrics.io_cost, metrics.cpu_cost);
//         return metrics;
//     }
...
//         metrics.io_cost = child_metrics.io_cost;
//         metrics.cpu_cost = child_metrics.cpu_cost + child_metrics.result_size * selectivity * 0.001;
        
//         if (table) free(table);
//         if (column) free(column);
//         if (op) free(op);
//         if (debugkaru) printf("Selection %s: size=%d, io=%.1f, cpu=%.1f\n",
//                              node->arg1, metrics.result_size, metrics.io_cost, metrics.cpu_cost);
//         return metrics;
//     }
    
//     if (strcmp(node->operation, "⨝") == 0) {
//         Node *left_table = NULL;
//         Node *right_table = NULL;
//         Node *current = node->child;
        
//         // Find first table
//         while (current && current->operation && strcmp(current->operation, "table") != 0) {
//             if (strcmp(current->operation, "σ") == 0 || strcmp(current->operation, "π") == 0) {
//                 current = current->child;
//             } else {
//                 break;
//             }
//         }
//         if (current && current->operation && strcmp(current->operation, "table") == 0) {
//             left_table = current;
//         }
        
//         // Find second table in next or child of left_table
//         if (left_table) {
//             if (left_table->next && left_table->next->operation && strcmp(left_table->next->operation, "table") == 0) {
//                 right_table = left_table->next;
//             } else if (left_table->child && left_table->child->operation && strcmp(left_table->child->operation, "table") == 0) {
//                 right_table = left_table->child;
//             } else {
//                 // Check siblings of the join node
//                 current = node->child;
//                 while (current && current != left_table) {
//                     if (current->next && current->next->operation && strcmp(current->next->operation, "table") == 0) {
//                         right_table = current->next;
//                         break;
//                     }
//                     current = current->child ? current->child : current->next;
//                 }
...
//         } else if (left_table->operation && (strcmp(left_table->operation, "σ") == 0 || strcmp(left_table->operation, "π") == 0)) {
//             Node *temp = left_table->child;
//             while (temp && temp->operation && strcmp(temp->operation, "table") != 0) {
//                 temp = temp->child;
//             }
//             left_name = temp ? temp->arg1 : NULL;
//         }
        
//         const char *right_name = NULL;
//         if (right_table->operation && strcmp(right_table->operation, "table") == 0) {
//             right_name = right_table->arg1;
//         } else if (right_table->operation && (strcmp(right_table->operation, "σ") == 0 || strcmp(right_table->operation, "π") == 0)) {
//             Node *temp = right_table->child;
//             while (temp && temp->operation && strcmp(temp->operation, "table") != 0) {
//                 temp = temp->child;
//             }
//             right_name = temp ? temp->arg1 : NULL;
//         }
        
//         if (debugkaru) printf("Join tables: left=%s, right=%s\n", left_name ? left_name : "NULL", right_name ? right_name : "NULL");
        
//         double selectivity = 1.0 / fmax(left_metrics.result_size, right_metrics.result_size);
...
//         metrics.io_cost = left_metrics.io_cost + right_metrics.io_cost;
//         metrics.cpu_cost = left_metrics.cpu_cost + right_metrics.cpu_cost + 
//                           (left_metrics.result_size * right_metrics.result_size * selectivity * 0.01);
...
//         if (debugkaru) printf("Projection %s: size=%d, io=%.1f, cpu=%.1f\n",
//                              node->arg1, metrics.result_size, metrics.io_cost, metrics.cpu_cost);
//         return metrics;
//     }
...
//         // Skip pushing projection through selection to avoid tree breakage
//         if (child->operation && strcmp(child->operation, "⨝") == 0 &&
//             child->child && child->child->operation && strcmp(child->child->operation, "table") == 0 &&
//             child->child->next && child->child->next->operation && strcmp(child->child->next->operation, "table") == 0) {
            
//             if (debugkaru) printf("\n[DEBUG] Case: Push projection through join\n");
            
//             Node *join_node = child;
//             Node *left_table = join_node->child;
//             Node *right_table = left_table->next;
            
//             if (debugkaru) printf("\n[DEBUG] Left table: %s, Right table: %s\n", 
//                                  left_table->arg1, right_table->arg1);
//             if (debugkaru) printf("\n[DEBUG] Join condition: %s\n", join_node->arg1);
