...
Node* duplicate_node(Node *node) {
    if (!node) return NULL;
    Node *new = new_node(node->operation, node->arg1, node->arg2);
    new->child = duplicate_node(node->child);
    new->next = duplicate_node(node->next);
    return new;
}
...
Node* optimize_query(Node *root) {
    ...
    double original_total = original_cost.io_cost + original_cost.cpu_cost;
    double selection_total = selection_cost.io_cost + selection_cost.cpu_cost;
    double projection_total = projection_cost.io_cost + projection_cost.cpu_cost;
    ...
    printf("Result Size     | %-13d | %-13d | %-13d | %s\n",
           ...
           best_plan_name);
    printf("I/O Cost        | %-13.1f | %-13.1f | %-13.1f | %s\n",
...
// Helper function to check if a node is a table scan
static int is_table_scan(Node *node) {
    return node && node->operation && strcmp(node->operation, "table") == 0;
}

// Helper function to get condition selectivity
