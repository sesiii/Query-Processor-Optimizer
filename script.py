...
def get_untracked_files():
    """Get a list of untracked (new) files, excluding a/ directory and script.py"""
    result = run_git_command(["git", "ls-files", "--others", "--exclude-standard"])
    files = [f for f in result.stdout.splitlines() if not f.startswith("a/") and f != "script.py"]
    print(f"Untracked files: {files}")
    return files

def get_modified_files():
    """Get a list of modified files, excluding script.py and files in a/"""
    result = run_git_command(["git", "status", "--porcelain"])
    files = [line.split()[-1] for line in result.stdout.splitlines()
             if line.startswith(" M ")
             and line.split()[-1] != "script.py"
             and not line.split()[-1].startswith("a/")]
    print(f"Modified files: {files}")
    return files

def get_deleted_files():
    """Get a list of deleted files, excluding script.py and files in a/"""
    result = run_git_command(["git", "status", "--porcelain"])
    files = [line.split()[-1] for line in result.stdout.splitlines()
