...
def get_untracked_files():
    ...
    files = [f for f in result.stdout.splitlines() if not f.startswith(("env/", "a/","script.py"))]
...
def get_modified_files():
    ...
    files = [line.split()[-1] for line in result.stdout.splitlines() if not line.endswith(("env/", "a/","script.py")) and line.startswith(" M ")]
...
def get_deleted_files():
    ...
    result = run_git_command(["git", "status", "--porcelain"])
    files = [line.split()[-1] for line in result.stdout.splitlines() if not line.endswith(("env/", "a/","script.py")) and line.startswith(" D ")]
